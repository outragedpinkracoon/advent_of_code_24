...
class DayOnePartOneTest < Minitest::Test
  def test_can_parse_data
    set_one, set_two = DayOnePartOne.parse('day_one/part_one/data/test_data.txt')
  ...
  def test_can_match_data
    set_one = [3, 4, 2, 1, 3, 3]
    set_two = [4, 3, 5, 3, 9, 3]
    diffs = DayOnePartOne.match(set_one, set_two)

    assert_equal([2, 1, 0, 1, 2, 5], diffs)
  end

  def test_can_return_the_correct_result
    result = DayOnePartOne.run('day_one/part_one/data/test_data.txt')
